             ...
             <tbody>
                 @forelse ($files as $file )
                     <tr>
                         ...
                         <td>
                             @if ($file->toAccountants == 0)
                                 <form action="{{ route('toAccounting', $file->id) }}" method="POST"
                                     enctype="multipart/form-data">
                                     @csrf
                                     <input type="hidden" name="_method" value="POST">
                                     <button class="btn btn-info btn-sm" value="submit"><i class="fas fa-paper-plane"
                                             style="color: success"></i> To
                                         Accounting</button>

                                 </form>
                             @else
                                 <a class="btn btn-info btn-sm"><i class="far fa-check-circle"></i>Sent</a>
